...
class ColorConverter {
  ...
  static Color rgb2color(int r, int g, int b) {
    ...
    return rgb2color2(r / 255.0, g / 255.0, b / 255.0);
  }

  /// Converts normalized RGB (0.0-1.0) values to a [Color] object.
  static Color rgb2color2(double r, double g, double b) {
    assert(r >= 0.0 && r <= 1.0,
        'r must be greater than or equal to 0 and less than or equal to 1');
    assert(g >= 0.0 && g <= 1.0,
        'g must be greater than or equal to 0 and less than or equal to 1');
  ...
  /// Converts a [Color] object to xy coordinates in the CIE 1931 color
  /// space.
  ///
  /// Returns a list of doubles representing the xy values. [x, y, brightness]
  static List<double> color2xy(Color color) {
    final int red = color.toRgbColor().r.toInt().clamp(0, 255);
    final int green = color.toRgbColor().g.toInt().clamp(0, 255);
    final int blue = color.toRgbColor().b.toInt().clamp(0, 255);

    return rgb2xy2(red / 255.0, green / 255.0, blue / 255.0);
  }
  ...
  static List<int> color2rgb(Color color) {
    final RgbColor rgbColor = color.toRgbColor();
    return [
      rgbColor.r.toInt().clamp(0, 255),
      rgbColor.g.toInt().clamp(0, 255),
      rgbColor.b.toInt().clamp(0, 255),
    ];
  }
  ...
  static int color2int(Color color) {
    final RgbColor colorObject = color.toRgbColor();

    final int a = 255 & 0xff;
    final int r = (colorObject.r).round() & 0xff;
    final int g = (colorObject.g).round() & 0xff;
    final int b = (colorObject.b).round() & 0xff;
